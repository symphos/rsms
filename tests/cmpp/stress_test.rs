...
impl ClientState {
    ...
    pub fn build_connect_pdu(&self) -> RawPdu {
        ...
        // 统一 Bind：version 字段承载握手版本（0x20/0x30）。
        let bind = UnifiedMessage::Bind(UnifiedBind {
            client_id: STRESS_TEST_ACCOUNT.to_string(),
...
/// 窄腰主路径：框架已按协商版本（V2.0/V3.0）decode，直接 match 统一消息枚举。
/// V2.0/V3.0 均走此路径——客户端预设 protocol_version 后框架自动按版本解码（D1a/D1b）。
#[async_trait]
...
impl MessageHandler for ServerHandler {
    ...
    async fn on_message(&self, ctx: &MessageContext, msg: &UnifiedMessage) -> Result<()> {
        // 窄腰主路径：框架已 decode，Submit → 回 SubmitResp + push report item。
        // ctx.reply 版本感知：V2.0 连接自动回 V2.0 SubmitResp（D1b），V3.0 回 V3.0 SubmitResp。
        if let UnifiedMessage::Submit(submit) = msg {
        ...
        for item in items {
            if let Some(acc) = account_pool.get(STRESS_TEST_ACCOUNT).await {
                if let Some(conn) = acc.get_connection_by_id(item.conn_id).await {
                    ...
                    let seq = rand_u32();
                    // 版本感知编码：V2.0 连接产 DeliverV20（60B 正文，registered_delivery=1），
                    // V3.0 连接产 V3.0 Deliver（71B 正文）；客户端按协商版本正确解码为 Report。
                    let cmpp_ver = if conn.protocol_version().await == Some(0x20) { CmppVersion::V20 } else { CmppVersion::V30 };
                    if let Ok(bytes) = CmppAdapter.encode_with_version(&report, Sequence::Plain(seq), cmpp_ver) {
        ...
        if let Some(acc) = account_pool.get(STRESS_TEST_ACCOUNT).await {
            if let Some(conn) = acc.first_connection().await {
                ...
                let deliver = UnifiedMessage::Deliver(UnifiedDeliver {
                    ...
                    dest: Address::plain(STRESS_TEST_ACCOUNT),
                ...
                let seq = rand_u32();
                // 版本感知编码：V2.0 连接产 DeliverV20（MO 上行），V3.0 产 V3.0 Deliver。
                let cmpp_ver = if conn.protocol_version().await == Some(0x20) { CmppVersion::V20 } else { CmppVersion::V30 };
                if let Ok(bytes) = CmppAdapter.encode_with_version(&deliver, Sequence::Plain(seq), cmpp_ver) {
                    if conn.write_frame(&bytes).await.is_ok() {
...
fn build_cmpp_submit_pdu(src: &str, dst: &str, content: &str, seq: u32, version: u8) -> Vec<u8> {
    if version == CMPP_VERSION_2_0 {
        ...
        // V3.0：统一 Submit；want_report=true 对应 registered_delivery=1；encoding=Gbk 对应 msg_fmt=15；
        // CMPP 方言（pk_*/msg_level/service_id/fee_*/msg_src）落 ProtocolExtra::Cmpp。
        let submit = UnifiedMessage::Submit(UnifiedSubmit {
            ...
            extra: ProtocolExtra::Cmpp(CmppExtra {
                ...
                fee_user_type: 0,
                fee_terminal_id: dst.to_string(),
                msg_src: "src".to_string(),
        ...
        CmppAdapter.encode(&submit, Sequence::Plain(seq)).expect("encode submit v30")
    }
}
