...
impl ClientState {
    fn new(stats: Arc<TestStats>, version: u8, account: AccountCredential) -> Self {
        Self {
...
impl MessageHandler for ClientState {
    ...
    async fn on_message(&self, ctx: &MessageContext, msg: &UnifiedMessage) -> Result<()> {
        // 窄腰主路径：框架已 decode，直接 match 统一消息枚举。
        match msg {
...
impl MessageHandler for ServerHandler {
    ...
    async fn on_message(&self, ctx: &MessageContext, msg: &UnifiedMessage) -> Result<()> {
        ...
        // 窄腰主路径：框架已 decode，直接 match Submit → 回 SubmitResp + push report item。
...
impl PasswordAuthHandler {
    ...
    fn add_account(mut self, account: &str, password: &str) -> Self {
...
async fn mt_producer_task(
    ...
    loop {
        ...
        let pdu_bytes = build_cmpp_submit_pdu(src, &account, &content, seq.fetch_add(1, Ordering::Relaxed) as u32);
...
#[tokio::test]
async fn stress_test_cmpp30_5accounts_5connections() {
    ...
    println!("\n");
    println!("==========================================================");
    println!("CMPP 3.0 Multi-Account Stress Test");
    println!("==========================================================");
    ...
    for (_idx, (account, password)) in ACCOUNTS.iter().enumerate() {
    ...
    print_stress_results(&results, "CMPP 3.0", "Multi-Account Stress Test");

    server_handle.abort();

    let actual_mt_qps = results.submit_sent as f64 / results.stress_secs;
    let expected_min = (MT_RATE_PER_ACCOUNT * NUM_ACCOUNTS as f64 * STRESS_TEST_DURATION_SECS as f64 * 0.4) as u64;
    assert!(results.submit_sent >= expected_min, "Expected at least {} MT, got {} ({:.1} QPS)", expected_min, results.submit_sent, actual_mt_qps);

    let match_ratio = if results.submit_resp > 0 { results.report_matched as f64 / results.submit_resp as f64 } else { 0.0 };
    assert!(
        results.report_matched >= results.submit_resp.saturating_sub(100),
        "Report match too low: {}/{} ({:.1}%)", results.report_matched, results.submit_resp, match_ratio * 100.0
    );
}
