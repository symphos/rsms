...
async fn create_connections(port: u16, count: usize) -> Vec<Arc<ClientConnection>> {
    ...
    for _ in 0..count {
        ...
        let login_pdu = build_login_pdu(TEST_ACCOUNT, TEST_PASSWORD);
        conn.write_frame(login_pdu.as_bytes()).await.expect("send login failed");
...
#[tokio::test]
async fn test_smgp_dynamic_connection_adjust_5_to_3() {
    ...
    println!("SMGP 5→3 发送 Submit: {}", sent);
    assert!(sent > 0);
    server_handle.abort();
}

#[tokio::test]
async fn test_smgp_dynamic_connection_multi_step() {
    let provider = Arc::new(DynamicConfigProvider::new(
        AccountConfig::new().with_max_connections(5).with_max_qps(1000).with_window_size(2048)
    ));
    let (port, account_pool, server_handle) = start_server(provider).await.unwrap();
    ...
    println!("SMGP 3→1 发送 Submit: {}", sent);
    assert!(sent > 0);
    server_handle.abort();
}
