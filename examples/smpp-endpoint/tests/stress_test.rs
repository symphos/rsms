            ...
            for _ in 0..batch_size {
                if let Some(msg_item) = queue.pop_front() {
                    let data = match msg_item {
                        MessageItem::Single(pdu) => pdu.as_bytes().to_vec(),
        ...
        if let Ok(msg) = decode_message(frame.data_as_slice()) {
            match msg {
                SmppMessage::SubmitSm(s) => {
                    ...
                    self.msg_source.push_item(STRESS_TEST_SYSTEM_ID, ReportItem {
                        msg_id,
                        conn_id: ctx.conn.id(),
                        dest_id,
                    }).await;
...
async fn report_generator_task(
    ...
    loop {
        ...
        let items = msg_source.fetch_items(STRESS_TEST_SYSTEM_ID, 100).await;
...
async fn run_stress_test(version: u8, num_connections: usize) {
    ...
    let monitor_handle = tokio::spawn({
        ...
        async move {
            ...
            loop {
                ...
                let queue_len = msg_source.queue_len(STRESS_TEST_SYSTEM_ID).await;
                ...
                println!("\n--- SMPP V{:02X} Stats at {:.1}s ---", version, elapsed);
    ...
    println!("\n");
    println!("==========================================");
    println!("SMPP Stress Test Results (V{:02X}, {} connections)", version, num_connections);
    println!("==========================================");
    println!("[时间]");
