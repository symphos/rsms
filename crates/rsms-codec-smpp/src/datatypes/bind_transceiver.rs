...
impl Decodable for BindTransceiver {
    fn decode(header: PduHeader, buf: &mut Cursor<&[u8]>) -> Result<Self, CodecError> {
        if header.command_id != CommandId::BIND_TRANSCEIVER {
...
impl Encodable for BindTransceiverResp {
    fn encode(&self, buf: &mut BytesMut) -> Result<(), CodecError> {
        encode_cstring(buf, &self.system_id, MAX_SYSTEM_ID_LENGTH)?;
        // sc_interface_version 是 SMPP 可选参数 TLV(0x0210, len 1)，不是裸字节——
        // 裸字节会被对端当作 TLV 头解析（需 4 字节却只有 1）导致 bind 失败
        // （联调实测：cmos 客户端报 NotEnoughDataInBufferException → login Failed）。
        buf.put_u16(0x0210);
        buf.put_u16(1);
...
impl Decodable for BindTransceiverResp {
    fn decode(header: PduHeader, buf: &mut Cursor<&[u8]>) -> Result<Self, CodecError> {
        if header.command_id != CommandId::BIND_TRANSCEIVER_RESP {
            return Err(CodecError::FieldValidation {
                field: "command_id",
                reason: "mismatch".to_string(),
            });
