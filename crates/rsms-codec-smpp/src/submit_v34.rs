...
use crate::version::{
    SMPP_V34_DEST_ADDR_SIZE, SMPP_V34_SERVICE_TYPE_SIZE, SMPP_V34_SOURCE_ADDR_SIZE,
};

pub fn decode_submit_sm_v34(_header_length: u32, body: &[u8]) -> Result<SubmitSm, RsmsError> {
    ...
    let service_type = decode_cstring(&mut cursor, SMPP_V34_SERVICE_TYPE_SIZE, "service_type")
        .map_err(|e| RsmsError::Codec(e.to_string()))?;
    let source_addr_ton = cursor.get_u8();
    let source_addr_npi = cursor.get_u8();
    let source_addr = decode_cstring(&mut cursor, SMPP_V34_SOURCE_ADDR_SIZE, "source_addr")
        .map_err(|e| RsmsError::Codec(e.to_string()))?;
    let dest_addr_ton = cursor.get_u8();
    let dest_addr_npi = cursor.get_u8();
    let destination_addr = decode_cstring(&mut cursor, SMPP_V34_DEST_ADDR_SIZE, "destination_addr")
        .map_err(|e| RsmsError::Codec(e.to_string()))?;
...
pub fn decode_deliver_sm_v34(_header_length: u32, body: &[u8]) -> Result<DeliverSm, RsmsError> {
    let mut cursor = Cursor::new(body);
